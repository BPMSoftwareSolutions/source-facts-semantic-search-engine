...
test("discovers the complete canonical-intent registry and projects queryable trace rows", async () => {
  ...
  assert.equal(discovery.pairs.length, 11);
  ...
  assert.equal(plane.canonicalFeatures.scenarios.length, 24);
  assert.equal(plane.canonicalFeatures.steps.length, 89);
  assert.equal(plane.canonicalIntents.responsibilities.length, 24);
