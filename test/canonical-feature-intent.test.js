...
for (const fixtureName of ["cli-project-command", "cli-query-command", "cli-call-graph-command", "cli-govern-command", "cli-propose-feature-coverage-command"]) {
...
test("discovers the complete CLI canonical-intent registry and projects queryable trace rows", async () => {
  ...
  assert.equal(discovery.pairs.length, 5);
  ...
  assert.equal(plane.canonicalFeatures.features.length, 5);
  assert.equal(plane.canonicalFeatures.scenarios.length, 6);
  assert.equal(plane.canonicalFeatures.steps.length, 24);
  assert.equal(plane.canonicalIntents.responsibilities.length, 6);
