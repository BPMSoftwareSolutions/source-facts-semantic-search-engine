...
test("discovers the complete canonical-intent registry and projects queryable trace rows", async () => {
  ...
  assert.equal(discovery.pairs.length, 12);
  ...
  assert.equal(plane.canonicalFeatures.features.length, 12);
  assert.equal(plane.canonicalFeatures.scenarios.length, 26);
  assert.equal(plane.canonicalFeatures.steps.length, 97);
  assert.equal(plane.canonicalIntents.responsibilities.length, 26);
