...
import { deterministicMechanicLowererVersion } from "../src/governance/lowers-deterministic-mechanic-authority.js";
...
test("queries bounded current mechanic candidates for deterministic lowering",async()=>{
  ...
  const rows=await queriesCurrentMechanicAuthorityCandidates({rootId:"root",mechanicKind:"branch",mechanicOccurrenceId:"m1",limit:5,lowererVersion:deterministicMechanicLowererVersion,connection,queryRunner:async request=>{query=request.query;return [`C|${JSON.stringify(candidate)}`];}});
