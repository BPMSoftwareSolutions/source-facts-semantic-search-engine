...
test("CLI-first closure inventories every command, classifies every callable, and isolates the unreachable remainder", async () => {
  ...
  assert.equal(projection.summary.observedCliCommandHandlers, 17);
  assert.equal(projection.summary.observedCliCommandTokens, 18);
  assert.equal(projection.summary.distinctCliExecutionSlices, 17);
  ...
  assert.equal(packets.rowCount, 17);
