...
test("CLI-first closure inventories every command, classifies every callable, and isolates the unreachable remainder", async () => {
  ...
  assert.equal(projection.summary.observedCliCommandHandlers, 19);
  assert.equal(projection.summary.observedCliCommandTokens, 20);
  assert.equal(projection.summary.distinctCliExecutionSlices, 19);
  ...
  assert.equal(packets.rowCount, 19);
