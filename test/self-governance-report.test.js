...
test("CLI-first closure inventories every command, classifies every callable, and isolates the unreachable remainder", async () => {
  ...
  assert.equal(projection.summary.observedCliCommandHandlers, 21);
  assert.equal(projection.summary.observedCliCommandTokens, 22);
  assert.equal(projection.summary.distinctCliExecutionSlices, 21);
  ...
  assert.equal(packets.rowCount, 21);
